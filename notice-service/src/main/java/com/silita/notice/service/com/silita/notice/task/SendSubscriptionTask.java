...
public class SendSubscriptionTask implements Runnable {
    ...
    public SendSubscriptionTask(Integer start, Integer pageSize, ElasticsearchService elasticsearchService, TbUserSubscribeMapper tbUserSubscribeMapper, TbMessageMapper tbMessageMapper, Date startTime, Date endTime) {
        this.start = start;
    ...
    private void saveSystemMsg(Map<String, Object> param, String userId, Integer pkid, Integer total, String title) {
        StringBuffer content = new StringBuffer("业务所在地：" + MapUtils.getString(param, "regionName") + "\n")
                .append("订阅关键词：" + MapUtils.getString(param, "keywords") + "\n")
                .append("资质：" + MapUtils.getString(param, "qualName") + "\n")
                .append("更新时间：" + DateUtils.dateToStr((Date) param.get("start"), "HH:mm") + "-" + DateUtils.dateToStr((Date) param.get("end"), "HH:mm") + "\n")
